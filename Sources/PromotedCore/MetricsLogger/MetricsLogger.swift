...
public final class MetricsLogger: NSObject {
  ...
  private(set) lazy var anonUserIDProducer: IDProducer = IDProducer(
    initialValueProducer: { [weak self] in
      guard let self = self else { return .null }
      if let savedID = self.store.anonUserID {
        return .autogenerated(value: savedID)
      }
      return self.idMap.anonUserID()
    },
    nextValueProducer: { [weak self] in self?.idMap.anonUserID() ?? .null }
  )
...
public extension MetricsLogger {
  ...
  var anonUserID: String? {
    get { anonUserIDProducer.currentValue.stringValue }
    set {
      anonUserIDProducer.currentValue = .idForPlatformSpecifiedString(newValue)
      history?.anonUserIDDidChange(value: newValue)
    }
  }
  ...
  /// `PeristentStore`.
  var currentOrPendingAnonUserID: String? {
    anonUserIDProducer.currentOrPendingValue.stringValue
  }
...
public extension MetricsLogger {
  ...
  /// persistent store, regenrates `anonUserID` and caches the new
  ...
  /// Updates `userID` to `nil` and generates a new `anonUserID`
  ...
  private func startSessionAndUpdateUserIDs(userID: String?) {
    ...
    // New session with same user should not regenerate anonUserID.
    ...
    // Reads anonUserID from store for initial value, if available.
    store.anonUserID = anonUserIDProducer.advance().stringValue
  }
}
...
extension MetricsLogger {
  ...
  /// - `userInfo` from `userID` and `anonUserID`
  ...
  private func logUser(properties: Message? = nil) -> Event_User? {
    ...
    monitor.execute {
      ...
      log(message: user)
      history?.anonUserIDDidChange(value: anonUserID, event: user)
