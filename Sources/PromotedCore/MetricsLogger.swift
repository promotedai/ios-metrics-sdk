...
public final class MetricsLogger: NSObject {
  ...
  init(deps: Deps) {
    ...
    logUserIDProducer = IDProducer(
      initialValueProducer: { [store, idMap] in
        if let savedID = store.logUserID {
          return .autogenerated(value: savedID)
        }
        return idMap.logUserID()
      },
      nextValueProducer: { [idMap] in idMap.logUserID() }
    )
...
extension MetricsLogger {
  private func deviceMessage() -> Common_Device {
    ...
    if let type = deviceInfo.deviceType.protoValue {
  ...
  private func identifierProvenanceMessage(
    ...
    if !config.eventsIncludeIDProvenance { return nil }
...
public extension MetricsLogger {
  ...
  func logImpression(
    ...
    monitor.execute {
      ...
      if let i = identifierProvenanceMessage(
        ...
        viewID: (
          viewID != nil ?
            .platformSpecified(value: viewID!) :
            viewTracker.id.currentValue
        )
      ) {
  ...
  internal func logView(
    ...
    monitor.execute {
      ...
      if let i = identifierProvenanceMessage(
        viewID: (
          viewID != nil ?
            .idForPlatformSpecifiedString(viewID) :
            viewTracker.id.currentValue
        )
      ) {
  ...
  func logAutoView(
    ...
    monitor.execute {
      ...
      if let i = identifierProvenanceMessage(autoViewID: autoViewID) {
